...
const SharePage = () => {
  ...
  return (
    <>
      {
        loading && <div className="h-[100vh] w-[100vw] bg-black z-50 fixed top-0 left-0 flex justify-center items-center">
          ...
          (id) ? (
            <>
              <div className='relative flex justify-center items-end'>
                ...
                <motion.img src={data?.profilePic} alt=" " className='mt-12 w-44 h-44 rounded-full z-10 sm:w-48 sm:h-48' initial={{
                  opacity: 0,
                  y: '35px'
                }} animate={{
                  opacity: 1,
                          ...
                          <div className="flex flex-col px-5 sm:px-10">
                            {
                              data.links.map((item: linkType) => {
                                if (!dropdownValue.slice(0, dropdownValue.length - 1).find((dropData) => {
                                  ...
                                  return <motion.div key={item.id} className='relative flex items-center justify-between my-1 border border-gray-500 rounded-lg p-3 cursor-pointer hover:bg-[#000d1d6b]' variants={divItemVariants}>
                                    <a href={item.siteURL} target='_blank' className=' w-[100%] flex items-center justify-between'>
                                    ...
                                    <div className='w-[70px] mr-2'>
                                      <ShareBtn header={item.siteName} description={`Share your ${item.siteName} link.`} link={item.siteURL} />
                                    </div>
                                  </motion.div>
                                }
                              })
