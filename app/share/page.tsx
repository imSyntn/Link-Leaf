...
const Share = () => {
  ...
  return (
    <>
      {
        !siteRendered && <div className="h-[100vh] w-[100vw] bg-black z-50 fixed top-0 left-0 flex justify-center items-center">
                      ...
                      <div className="flex flex-col px-5 sm:px-10">
                        {
                          data.links?.map((item: linkType, index: number) => {
                            if (!dropdownValue.slice(0, dropdownValue.length - 1).find((dropData) => {
                              ...
                              return <motion.div key={item.id} className='relative flex items-center justify-between my-1 border border-gray-500 rounded-lg p-3 cursor-pointer' variants={divItemVariants}>
